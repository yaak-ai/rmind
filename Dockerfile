...
RUN apt-get update && apt-get install -y --no-install-recommends \
        curl ca-certificates build-essential libexpat1 ffmpeg \
...
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --extra train --extra predict --locked --no-install-project

# source + config templates
COPY . .
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --extra train --extra predict --locked
