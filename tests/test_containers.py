...
def test_moduledict() -> None:
    ...
    assert (transforms.forward(x) == TensorDict({"image": transform(x)})).all()  # pyright: ignore[reportAttributeAccessIssue]
    ...
    assert (
        ...
        == TensorDict({
            "image": {"cam_front_left": transform(x["image"]["cam_front_left"])}  # pyright: ignore[reportArgumentType]
        })
