...
def test_weights_mean_one_over_empirical(tmp_path: Path) -> None:
    ...
    w = ManeuverLossWeights.from_stats_file(_stats_file(tmp_path), alpha=1.0, cap=1e9)
...
def test_alpha_zero_is_uniform(tmp_path: Path) -> None:
    ...
    w = ManeuverLossWeights.from_stats_file(_stats_file(tmp_path), alpha=0.0, cap=1e9)
    assert_close(w.bin_weight, torch.ones(2, NBINS), atol=1e-5, rtol=0)


def test_cap_bounds_weights(tmp_path: Path) -> None:
    cap = 2.0
    w = ManeuverLossWeights.from_stats_file(_stats_file(tmp_path), alpha=1.0, cap=cap)
...
def test_lookup_buckets_and_clamps(tmp_path: Path) -> None:
    w = ManeuverLossWeights.from_stats_file(_stats_file(tmp_path), alpha=1.0, cap=1e9)
    label = torch.tensor([[0.01, 0.5], [0.99, 0.5], [5.0, 0.5]])  # last row out-of-range
    out = w(label)
...
def test_rejects_wrong_channel_count(tmp_path: Path) -> None:
    w = ManeuverLossWeights.from_stats_file(_stats_file(tmp_path), alpha=0.5, cap=15.0)
