...
from rmind.components.norm import Normalize
...
class EpisodeBuilder(Module):
    ...
    def forward(self, batch: TensorTree) -> Episode | EpisodeExport:
        ...
        if batch["phase"] == "predict":
            self.modality_dropouts["image"].probability = 0
            self.modality_dropouts["continuous"]["speed"].probability = 0
            self.modality_dropouts["context"]["waypoints"].probability = 1
            input_embeddings = self.modality_dropouts(input_embeddings)
