"""Per-chunk maneuver loss weighting via Label Distribution Smoothing.
...
flow_action_thresholds.py fits and writes {edges, emp, smooth, model_keys} under
...
class ManeuverLossWeights(nn.Module):
    ...
    def from_stats_file(cls, path: str | Path, *, alpha: float, cap: float) -> Self:
        ...
        if "lds" not in stats:
            msg = f"no 'lds' section in stats file {path} (fit with +thresholds.lds=true)"
            raise KeyError(msg)
        lds = stats["lds"]
        return cls(
    ...
    def forward(self, label: Tensor) -> Tensor:
        ...
        for c in range(self.model_dim):
            ...
            out[..., c] = self.bin_weight[c][idx.clamp_(0, nbins - 1)]
        return out
