...
FlowSamplingMethod = Literal["euler", "heun"]
...
class AdaLNCrossAttentionDecoderBlock(nn.Module):
    ...
    def forward(self, x: Tensor, context: Tensor, time_emb: Tensor) -> Tensor:
        # time_emb is (B, time_dim), broadcast over slots (unsqueeze the slot
        # axis); a (B, H, time_dim) per-slot modulation is also accepted as-is.
        mods = self.adaLN_modulation(time_emb).chunk(6, dim=-1)
        if time_emb.dim() == 2:  # noqa: PLR2004
...
class FlowActionDecoder(nn.Module):
        ...
        for step in range(steps):
            ...
            match method:
                ...
                case "heun":
                    ...
                    # out-of-place (not `x +=`, hence noqa): on the first step
                    # `x` is the caller's noise, and in-place would mutate it as
                    # a side effect (and is harder to export). Matches euler.
                    x = x + 0.5 * dt * (velocity + corrected_velocity)  # noqa: PLR6104
    ...
    def _validate_sampling_method(method: str) -> None:
        if method not in {"euler", "heun"}:
            msg = (
                f"flow sampling method must be one of ('euler', 'heun'), got {method!r}"
            )
            raise ValueError(msg)
    ...
    def _prepare_flow_time(
        *, flow_time: Tensor, batch_size: int, dtype: torch.dtype, device: torch.device
        ...
        if flow_time.ndim != 1 or flow_time.shape[0] != batch_size:
            msg = (
                "flow_time must have one value per batch item, "
