...
class LogitBiasSetter(Callback):
    ...
    def on_fit_start(  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        keypaths, _ = tree_flatten_with_path(
            pl_module.input_builder[0].paths,  # pyright: ignore[reportIndexIssue]
        ...
        batch = trainer.train_dataloader.dataset.get_batch(  # pyright: ignore[reportOptionalMemberAccess]
            slice(-1), keys=batch_keys
        ).to_tensordict()

        with torch.inference_mode():
            input = pl_module.input_builder.forward(
            ...
            labels = pl_module.episode_builder.tokenizers.forward(input)  # pyright: ignore[reportAttributeAccessIssue]
