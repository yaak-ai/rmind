...
class PatchPolicy(pl.LightningModule, LoadableFromArtifact):
    ...
    def _trunk_token_norms(
        ...
        `[speed, camera patches..., register_0.., READOUT]` layout built by
        `_frame_tokens` still indexes it. The boundaries are derived from the
        frame width, `num_register_tokens` and `use_readout_token` (and
        `len(self.cameras)` for the per-camera split), never hardcoded.
        ...
        with torch.no_grad():
            ...
            # per-camera: a single camera going dark (frozen frames, a dead
            # feed) moves its own block and nothing else, which the pooled
            # `out/patch` mean hides. Camera `c` owns the patch-block slice
            # `[1 + c*p, 1 + (c+1)*p)` -- see `_frame_tokens`, which
            # concatenates the cameras along the patch axis in `self.cameras`
            # order.
            if len(self.cameras) > 1:
                per_camera = (patch_end - 1) // len(self.cameras)
                for index, camera in enumerate(self.cameras):
                    start = 1 + index * per_camera
                    token_norms[f"out/patch/{camera}"] = norms[
                        ..., start : start + per_camera
                    ].mean()
