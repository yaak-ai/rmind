...
class ControlTransformer(pl.LightningModule, LoadableFromArtifact):
        ...
        **kwargs: Any,
        ...
        match hparams_updaters:
            ...
            case _:
                ...
                return model.to(device)  # ty: ignore[possibly-unbound-attribute]
    ...
    def training_step(self, batch: dict[str, Any], _batch_idx: int) -> Tensor:
        ...
        metrics = TensorDict({
            name: objective.compute_metrics(episode)
            for name, objective in self.objectives.items()
        })

        losses = metrics.select(*((k, "loss") for k in metrics.keys()))  # noqa: SIM118, # ty: ignore[not-iterable]
    ...
    def forward(self, batch: TensorTree) -> TensorTree | TensorDict:
        ...
        return TensorDict(outputs) if not torch.compiler.is_exporting() else outputs  # ty: ignore[possibly-unbound-attribute]
