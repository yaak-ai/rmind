...
class ControlTransformer(pl.LightningModule, LoadableFromArtifact):
    ...
    def training_step(self, batch: TensorDict, _batch_idx: int) -> Tensor:
        ...
        losses = TensorDict({
            name: self.objectives[name].compute_losses(episode)  # pyright: ignore[reportCallIssue]
        ...
        losses["total"] = losses.sum(reduce=True)
    ...
    def validation_step(self, batch: TensorDict, _batch_idx: int) -> Tensor:
        ...
        episode = self.episode_builder.forward(input)
        losses = TensorDict({
            name: objective.compute_losses(episode)  # pyright: ignore[reportCallIssue]
            for name, objective in self.objectives.items()
        })
        ...
        if not self.trainer.sanity_checking:
            self.log_dict(
                {
                    "/".join(["val", "loss", *k]): v
                    for k, v in losses.items(include_nested=True, leaves_only=True)
                },
                sync_dist=True,
            )
