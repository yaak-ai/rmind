...
class Trajectory(pl.LightningModule):
    ...
    def get_model_trajactories(self, batch, batch_idx):
        preds_last = self.model.predict_step(
            batch, batch_idx=batch_idx, start_timestep=-1
        )
    ...
    def predict_step(
        ...
        model_actions = self.get_model_trajactories(batch, batch_idx=batch_idx)
        ...
        *,
        ...
        for i in range(clips):
            ...
            curr_points = self.calculate_trajectory(
                ...
                acceleration_x=VehicleMotion_acceleration_x[i].item(),
