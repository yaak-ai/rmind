...
class Frames(pl.LightningModule):
    ...
    def get_future_frames(self, batch: Any) -> List[np.ndarray]:
        ...
        # timestep_size = self.hparams.patch_row * self.hparams.patch_col + len(
        #     self.model.metadata_keys + ["sep"] + self.model.action_keys
        # )
        ...
        with torch.no_grad():
            ...
            #     F.softmax(logits[:, [-1]], -1).squeeze(1), 1
            # )
            ...
            for timestep in tqdm.tqdm(range(self.hparams.future_timestep), ascii=True, unit="img"):  # type: ignore
                ...
                # Update sample to have last predicted image as part of frames
                # Update last predicted metadata and actions keys as well
                # input_images = sample["frames"].clone()
                # sample["frames"][:, : clips - 1] = input_images[:, 1:]
                # sample["frames"][:, [clips - 1]] = map_pixels(image)
