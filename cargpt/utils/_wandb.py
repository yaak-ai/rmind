...
class TrainValAttnMapLoggingMixin:
    ...
    def __should_log_now(self, batch_idx):
        params: Dict[str, Any] = self.__get_params()
    ...
    def __get_attention_maps(self, x, mask=None):
        attention_maps = []
        with torch.no_grad():
    ...
    def __logits_to_real(self, logits: Float[Tensor, "to e"]) -> List[float]:
    ...
    def _log_attn_maps(
        ...
        **kwargs,
        ...
        if not self.__is_logging_active():
            return

        if not self.__should_log_now(batch_idx):
            return

        # Pick only one sample from batch - consistent with deephouse approach
        batch_idx = 0

        attn_maps: List[Float[Tensor, "to to"]] = self.__get_attention_maps(
            episode[batch_idx],
            episode_mask,
        )

        batch_frames: Float[Tensor, "ts c h w"] = frames[batch_idx]
        images, metas_actions = self.__postprocess_attn_maps(
            attn_maps=attn_maps,
            frames=batch_frames,
        ...
        pred_actions_values = self.__logits_to_real(logits[batch_idx])

        stage = self.trainer.state.stage
        data: Dict[str, Any] = self.__get_image_data(
            gt_actions_values=gt_actions_values,
            pred_actions_values=pred_actions_values,
        ...
        data.update(self.__get_histograms_data(attn_maps=attn_maps))
    ...
    def __postprocess_attn_maps(
        ...
        meta_sep_act_len = len(self.action_keys) + 1 + len(self.metadata_keys)
        seq_len = (tokens + 1) // timesteps
        images_len = seq_len - meta_sep_act_len
        cols = params["cols"]
        rows = params["rows"]

        last_meta_actions = self.__extract_single_attn_map(attn_maps)
        ...
        for idx, row in enumerate(last_meta_actions):
            ...
            row = self.__preprocess_attn_row(row_vector=row)
            ...
            for ts in range(timesteps):
                ...
                row_images.append(
                    fromarray(self.__get_masked_img(mask=mask, frame=frames[ts]))
                )
                ...
                row_metas_actions.append(
                    fromarray(
                        self.__get_meta_and_actions_img(
                            meta_actions=ma, length=meta_sep_act_len
                        )
                    )
                )
    ...
    def __extract_single_attn_map(
        ...
        last_meta_actions = attn_map[-len(self.action_keys) :]
        return last_meta_actions

    def __preprocess_attn_row(
        self, row_vector: Float[Tensor, "to"]
    ) -> Float[Tensor, "to"]:
        params = self.__get_params()
        transforms = params.get("row_transform")
    ...
    def __get_masked_img(self, mask, frame):
        _, img_height, img_width = frame.shape
        params = self.__get_params()
        strength = params.get("strength") or 1.0
        reverse = params.get("reverse") or False

        color = params.get("color") or [255, 255, 255]
        color = rearrange(torch.tensor(color).to(frame) / 255.0, "(c 1 1) -> c 1 1")
    ...
    def __get_meta_and_actions_img(self, meta_actions, length):
        ma = meta_actions
        if len(ma) < length:
            tmp = torch.zeros(length).to(ma)
            tmp[: len(ma)] = ma
            ma = tmp
        ma = ma[None, ...]
    ...
    def __get_histograms_data(
        self, attn_maps: List[Float[Tensor, "a to"]]
        ...
        last_meta_actions = self.__extract_single_attn_map(attn_maps)
        data = {}
        for idx, vector in enumerate(last_meta_actions):
            weights = vector.clone()
            histogram = wandb.Histogram(weights.detach().cpu(), num_bins=512)  # type: ignore
            key = f"{self.trainer.state.stage}-histograms/{self.action_keys[idx]}"
            data[key] = histogram
        return data
